...
from db.repository.blog import delete_blog_by_ids
...
def update_a_blog(id: int, blog: UpdateBlog, db: Session = Depends(get_db)):
    blog = update_blog(id=id, updated_blog=blog, author_id=1, db=db)
    if not blog:
...
def delete_blog(id: int, author_id: int, db: Session = Depends(get_db)):
    ...
    if response.get("error"):
        raise HTTPException(
            detail="An error has ocurred", status_code=status.HTTP_404_NOT_FOUND
        )
